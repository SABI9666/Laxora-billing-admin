...
export default function AdminLedgerPage() {
  ...
  useEffect(() => {
    api<Ledger>(`/api/admin/parties/${id}/ledger`).then(setData);
  }, [id]);

  if (!data) return <div className="p-8 text-gray-400">Loading statement…</div>;

  const business = data.business ?? { name: data.party.name };
