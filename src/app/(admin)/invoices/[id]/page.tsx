...
export default function AdminInvoicePage() {
  ...
  useEffect(() => {
    api<{ invoice: Invoice }>(`/api/admin/invoices/${id}`).then((r) => setInvoice(r.invoice));
  }, [id]);

  if (!invoice) return <div className="p-8 text-gray-400">Loading invoice…</div>;
  ...
  const halfTax = tax / 2; // intra-state CGST/SGST split
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="invoice-sheet rounded-xl border border-gray-300 bg-white p-8 text-[13px] leading-relaxed text-gray-900">
        ...
        {invoice.payments.length > 0 && (
          <div className="no-print mt-3 rounded-lg bg-gray-50 p-3 text-xs text-gray-600">
            <p className="mb-1 font-semibold uppercase text-gray-400">Payments received</p>
            {invoice.payments.map((p) => (
              <p key={p.id}>
                {formatDate(p.paymentDate)} · {p.method} · {formatMoney(p.amount)}
              </p>
            ))}
          </div>
        )}
